...
public static class EventManager
{
    ...
    //Difficulty Menu Support
    static Dictionary<EventName, List<IntEventInvoker>> invokers = new Dictionary<EventName, List<IntEventInvoker>>();

    static Dictionary<EventName, List<UnityAction<int, GameType>>> listeners = 
        new Dictionary<EventName, List<UnityAction<int, GameType>>>();
    ...
    public static void Initialize()
    {
        foreach(EventName name in Enum.GetValues(typeof(EventName)))
        {
            if(!invokers.ContainsKey(name))
            {
                ...
                listeners.Add(name, new List<UnityAction<int, GameType>>());
            }
            else
            {
                invokers[name].Clear();
    ...
    {
        foreach(UnityAction<int, GameType> listener in listeners[eventName])
        {
    ...
    public static void AddListener(EventName eventName, UnityAction<int, GameType> listener)
    {
